...
function draw(zoom) {
  ...
  ctx.fillText("Entering Committee Space...", 50, 100);
  ctx.restore();
}
...
gsap.to({ z: 1 }, {
  z: 2,
  scrollTrigger: {
    trigger: ".committees-section",
    start: "top top",
    end: "bottom bottom",
    scrub: true
  },
  onUpdate(self) {
    zoom = self.targets()[0].z;
    draw(zoom);
  }
});

gsap.utils.toArray(".committee-panel").forEach((panel, i) => {
  gsap.to(panel, {
    scrollTrigger: {
      trigger: ".committees-section",
      start: `${i * 100 + 10}% center`,
      end: `${(i + 1) * 100}% center`,
      scrub: true,
    },
    opacity: 1,
    y: 0, // animate back to natural position
    duration: 1,
    ease: "power2.out"
  });
});
